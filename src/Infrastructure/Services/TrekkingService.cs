...
public sealed class TrekkingService(AppDbContext db) : ITrekkingService
{
    public async Task<TrekkingPagedResult> ListAsync(string? search, string? status, string? destination, string? trekkingType, bool? featured, int page, int pageSize, CancellationToken ct)
    {
        ...
        var trekkingTypeFilters = await db.TrekkingTypes.AsNoTracking()
            .OrderBy(t => t.Title)
            .Select(t => new TrekkingTypeFilterOptionDto(
                t.Id.ToString(),
                t.Title,
                t.Country.ToString(),
                trekkingType != null && string.Equals(trekkingType, t.Id.ToString(), StringComparison.OrdinalIgnoreCase)))
            .ToListAsync(ct);

        return new TrekkingPagedResult(items, page, pageSize, total, locations, trekkingTypes, trekkingTypeFilters);
    }

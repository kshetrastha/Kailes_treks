...
public sealed class TrekkingService(AppDbContext db) : ITrekkingService
{
    public async Task<TrekkingPagedResult> ListAsync(string? search, string? status, string? destination, string? trekkingType, bool? featured, int page, int pageSize, CancellationToken ct)
    {
        ...
        var trekkingTypesQuery = db.TrekkingTypes.AsNoTracking();
        if (!string.IsNullOrWhiteSpace(destination) && Enum.TryParse<Country>(destination, true, out var selectedCountry))
        {
            trekkingTypesQuery = trekkingTypesQuery.Where(t => t.Country == selectedCountry);
        }

        var trekkingTypes = await trekkingTypesQuery
            .OrderBy(t => t.Title)

...
{
    [HttpGet("")]
    public async Task<IActionResult> Index(string? search, string? destination, string? difficulty, int page = 1, int pageSize = 10, CancellationToken ct = default)
    {
        ...
        return View(await service.ListAsync(search, null, destination, null, currentPage, currentPageSize, ct));
    }
